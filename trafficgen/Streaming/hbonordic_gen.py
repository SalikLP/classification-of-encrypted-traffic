...
def enable_flash(page):
    ...
    shadow_settings_privacy_page = expand_shadow_element(settings_privacy_page)
    ...
    shadow_category_setting_exceptions = expand_shadow_element(category_setting_exceptions)
    site_list = shadow_category_setting_exceptions.find_elements_by_css_selector("site-list[category-header='Tillad']")[
        0]
    shadow_site_list = expand_shadow_element(site_list)
    ...
    shadow_dialog = expand_shadow_element(dialog)
...
for i in range(5):
    ...
    duration = 60 * 1
    enable_flash(page="https://dk.hbonordic.com:443")


    _thread.start_new_thread(captureTraffic,(1, duration, 'C:/users/arhjo/desktop/test', "HBO"))

    login("emilipsen@hotmail.com", "folkmann4")
